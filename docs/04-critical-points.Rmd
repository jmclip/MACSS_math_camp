...
function_plot <- ggplot(data.frame(x = c(0, 5)), aes(x))

function_plot <- ggplot(data.frame(x = c(0, 5)), aes(x))

fx <- function_plot +
  stat_function(fun = function(x) (x^3 - 15 * x^2 + 60 * x + 30) / 15,
                n = 201, linewidth = 0.5) +
  geom_abline(slope = -1, intercept = (10 + (1 / 3)),
              linetype = 4, color = "blue", linewidth = 0.8) +
  labs(y = expression(f(x) == frac(x^3 - 15 * x^2 + 60 * x + 30, 15))) +
  coord_cartesian(xlim = c(0, 5), ylim = c(0, 8)) +
  theme_minimal()

fx1 <- function_plot +
  stat_function(fun = function(x) (x^2 / 5) - 2 * x + 4,
                n = 201, linewidth = 0.5) +
  ...
  coord_cartesian(xlim = c(0, 5), ylim = c(-2, 8)) +
  theme_minimal()
...
* Where a function is twice differentiable and concave over some area, then the function is **concave down where $f''(x) < 0$** and where $f''(x) > 0$ is **concave up**. Recall: $f(x)=(x^3 - 15 * x^2 + 60 * x + 30) / 15$.
...
How would we evaluate the extreme values here? 
```{r endpoints2, fig.width=8, fig.height=6, out.width="25%"}
function_plot +
  stat_function(fun = function(x) x, linewidth = .5) +
  geom_point(
    data = tibble(
...
* Maximize the log-likelihood instead (easier math!)
...
Logit and other limited-dependent variable models: what parameter estimate makes the observed pattern most likely?

* Simplest example: 0s and 1s as outcome
* Trying to determine which value of $\beta$ best predicts the outcomes observed
...
* **Question** Do you make a zero vector with *some* combo of all of these?
...
$$
\begin{aligned}
4\alpha + 2\beta + 1\gamma &= 0 (h)\\
3\alpha + 1\beta + 1\gamma &= 0 (j)\\
3\alpha - 1\beta + 2\gamma &= 0 (k)
\end{aligned}
$$

--

* **Step 2: manipulate (j-k)**
$$
\begin{aligned}
