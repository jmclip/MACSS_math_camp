...
$X$ and $Y$ are **jointly continuous** if there exists a function $f(x,y)$ such that, for every set $C \subset \mathbb{R}^{2}$,
...
Consider a function $f:\mathbb{R} \times \mathbb{R} \rightarrow \mathbb{R}$:
...
$f(x,y) = \frac{2 \exp(-2x)}{\sqrt{2\pi}}\exp\left(-\frac{(y)^2}{2}\right)$ if $x \in [0,\infty), y \in \mathbb{R}$, $f(x,y) = 0$ otherwise
...
$f(x,y) = \phi(x)\,\phi(y)$, the product of two standard normal densities
...
$$f(x, y) = \phi(x)\,\phi(y) \quad \text{(independent standard normals)}$$
.pull-left[
...
function_plot +
  geom_line(
    data = z,
    mapping = aes(x = x1, y = y, color = y), linewidth = 1.5
  ) +
  scale_color_viridis_c() +
  labs(
    title = expression(f(x, y) == phi(x) * phi(y)),
...
We want $f_{X}(x)$. We can find this one of two ways: by integrating the joint density over $y$, OR by averaging the conditional densities, $f_{X}(x) = \int f(x|y) f_{Y}(y)\,dy$. 
...
$$\begin{aligned}f(x,y) &  = & x + y \text{ , if  } x \in [0,1], y \in [0,1] \\& = & 0 \text{ , otherwise }  \end{aligned}$$
...
Suppose $g:\mathbb{R}^{2} \rightarrow \mathbb{R}$. Then
...
\end{align}$$
--
PHEW! It's a pdf. And it is non-negative on the region: the minimum over $x \in [1,2], y \in [0,0.5]$ is at $(1, 0.5)$, where $f = 1 - \frac{4}{3} + \frac{1}{3} = 0$.
...
$$\begin{aligned}\Cov(X,X) & = & E[X X] - E[X]E[X] \\& = & E[X^2] - E[X]^2\end{aligned}$$
...
$$\begin{aligned}\Cor(X,Y) & = & \frac{\Cov(X,Y)}{\sqrt{\Var(X)\Var(Y)} } \\& = & \frac{\Var(X)}{\Var(X)} \\& = & 1\end{aligned}$$
...
We can keep going with the dimensions!
